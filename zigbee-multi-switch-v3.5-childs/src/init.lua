  ...
  for id, value in pairs(device.preferences) do
    ...
    if oldPreferenceValue ~= newParameterValue then
      ...
      elseif id == "switch2Child" then
        if oldPreferenceValue ~= nil and newParameterValue == "Yes" then
          if device:get_model() == "UPSK2O" then
            child_devices.create_new_1(driver, device, "switch2")
          else
      ...
      -- elseif id == "smartplug2Child" then
      --   if oldPreferenceValue ~= nil and newParameterValue == "Yes" then
      --     child_devices.create_new_1(driver, device, "smartplug2")
      --   end
      end
    end
  end
...
local function all_switches_status(driver,device)
  ...
  print("all_switches_status >>>>>")
   for id, value in pairs(device.preferences) do
    ...
    --  if device:get_latest_state("smartplug2", capabilities.switch.ID, capabilities.switch.switch.NAME) == "on" then
    --   total_on = total_on + 1
    --   status_Text = status_Text.."S2:On "
    -- end
     --print("Total_on >>>>>>", total_on,"Total >>>",total)
